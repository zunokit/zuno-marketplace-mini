...
async function listNFT() {
  ...
  try {
    ...
    const isApproved = await nftContract.isApprovedForAll(signerAddress, exchangeAddress);
    ...
    const exchangeAbi = [
      'function listNFT(address,uint256,uint256,uint256) returns (bytes32)' + (isERC1155 ? '' : ''),
      'function listNFT(address,uint256,uint256,uint256,uint256) returns (bytes32)' + (isERC1155 ? '' : ''),
    ...
    const listingEvent = receipt.logs.find(
      log => log.topics[0] === ethers.id('NFTListed(bytes32,address,uint256,address,uint256,uint256,address,uint256)')
    );
    ...
    if (listingEvent) {
      ...
      console.log(`   View on marketplace: [Your marketplace URL]/listing/${listingId}`);
    } else {
      console.log(`\n✅ NFT Listed Successfully!`);
      console.log(`   Transaction: ${tx.hash}`);
    }
