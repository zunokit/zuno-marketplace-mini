...
async function createBundle() {
  ...
  try {
    ...
    for (let i = 0; i < numNfts; i++) {
      ...
      if (isERC721) {
        ...
        if (owner.toLowerCase() !== sellerAddress.toLowerCase()) {
          throw new Error(`You don't own token ID ${tokenId} from ${nftAddress}`);
        }
        console.log(`  ✅ ERC721 - ${name || 'Unknown'} (${symbol || '???'})`);
      } else if (isERC1155) {
        const balance = await nftContract.balanceOf(sellerAddress, tokenId);
        if (balance === 0n) {
          throw new Error(`You don't own any tokens of ID ${tokenId} from ${nftAddress}`);
        }
        
        const amountStr = await prompt(`  Amount to bundle (you have ${balance}): `);
        ...
        console.log(`  ✅ ERC1155 - ${name || 'Unknown'} (${symbol || '???'}) x${amount}`);
      } else {
    ...
    bundleItems.forEach((item, i) => {
      console.log(`     ${i + 1}. ${item.name} #${item.tokenId}${item.amount > 1 ? ` x${item.amount}` : ''}`);
    });
    ...
    const bundleManagerAddress = await hub.getBundleManager();
    ...
    for (const item of bundleItems) {
      ...
      if (item.tokenType === 0) {
        ...
        const isApprovedForAll = await nftContract.isApprovedForAll(sellerAddress, bundleManagerAddress);
        
        if (approved.toLowerCase() !== bundleManagerAddress.toLowerCase() && !isApprovedForAll) {
          console.log(`   Approving ${item.name} #${item.tokenId}...`);
          const tx = await nftContract.setApprovalForAll(bundleManagerAddress, true);
          await tx.wait();
          console.log(`   ✅ Approved`);
        }
      } else {
        // ERC1155 - check setApprovalForAll
        const isApproved = await nftContract.isApprovedForAll(sellerAddress, bundleManagerAddress);
    ...
    const bundleEvent = receipt.logs.find(
      log => log.topics[0] === ethers.id('BundleCreated(bytes32,address,tuple(address,uint256,uint256,uint8)[],uint256,uint256)')
    );
