...
async function testMarketplace() {
  ...
  try {
    ...
    const match721 = output721.match(/Collection deployed at: (0x[a-fA-F0-9]{40})/);
    if (match721) {
    ...
    const match1155 = output1155.match(/Collection deployed at: (0x[a-fA-F0-9]{40})/);
    if (match1155) {
    ...
    if (erc721Address) {
      ...
      log('   Note: This will prompt for auction parameters', 'yellow');
      log('   Suggested values:', 'yellow');
      log('     - Auction type: 1 (English)', 'yellow');
      log('     - Starting price: 0.05 ETH', 'yellow');
      log('     - Reserve price: 0.1 ETH', 'yellow');
      log('     - Duration: 1 day', 'yellow');
      
      // Note: Can't fully automate interactive script
      log('\n   ⚠️  Skipping interactive auction creation in automated test', 'yellow');
    }
    ...
    log('   Note: Offer creation is interactive', 'yellow');
    log('   ⚠️  Skipping interactive offer creation in automated test', 'yellow');
    ...
    log('   Note: Bundle creation is interactive', 'yellow');
    log('   ⚠️  Skipping interactive bundle creation in automated test', 'yellow');
    ...
    if (erc1155Address) log(`   ERC1155 Collection: ${erc1155Address}`, 'green');
    ...
    log('   - Auction creation: node scripts/auctions/create-auction.js', 'cyan');
