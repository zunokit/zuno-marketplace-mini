    ...
    <Link href={`/auctions/${auction.id}`}>
      <Card className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer">
        <div className="h-40 bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center relative">
          <span className="text-4xl font-bold text-primary/30">#{auction.tokenId}</span>
          <Badge 
        ...
        <CardContent className="p-4">
          <p className="text-xs text-muted-foreground font-mono mb-2">
            {auction.collectionAddress.slice(0, 10)}...{auction.collectionAddress.slice(-8)}
          </p>
          ...
          <div className="flex items-center gap-1 text-sm text-muted-foreground">
            ...
            <span>
              {timeLeft > 0 ? `${hours}h ${minutes}m left` : 'Ended'}
            </span>
          </div>
        </CardContent>
      </Card>
...
export default function AuctionsPage() {
  ...
  const { data: activeAuctions, isLoading: loadingActive } = useActiveAuctions(1, 50);
  const { data: myAuctions, isLoading: loadingMy } = useAuctionsBySeller(address, 1, 50);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Tabs value={tab} onValueChange={setTab} className="mb-6">
          ...
          <TabsContent value="my" className="mt-6">
            {!address ? (
              <div className="text-center py-12">
                <p className="text-muted-foreground">Connect wallet to view your auctions</p>
              </div>
            ) : loadingMy ? (
              <div className="flex justify-center py-12">
                <Loader2 className="h-8 w-8 animate-spin text-primary" />
              ...
              <div className="text-center py-12">
                <p className="text-lg text-muted-foreground mb-4">You have no auctions</p>
                <Button asChild>
