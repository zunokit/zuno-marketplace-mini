...
import { COLLECTION_FACTORY_ABI, ERC721_ABI, ERC1155_ABI } from '@/lib/contracts/abis'
...
export class CollectionService extends BaseContractService {
  constructor(chainId: number = 31337) {
    const contractAddress = getContractAddress('COLLECTION_FACTORY', chainId)
  ...
  async createERC721Collection(
    ...
    return await this.sendTransaction('createERC721Collection', name, symbol, baseURI)
  }
  ...
  async createERC1155Collection(
    ...
    return await this.sendTransaction('createERC1155Collection', name, symbol, baseURI)
  }
   ...
   */
  async getERC721CollectionDetails(collectionAddress: string): Promise<Collection | null> {
    try {
   ...
   */
  async getERC1155CollectionDetails(collectionAddress: string): Promise<Collection | null> {
    try {
      if (!this.provider) throw new Error('Provider not initialized')
   ...
   */
  async getUserCollectionsWithDetails(userAddress: string): Promise<Collection[]> {
    try {
      const collectionAddresses = await this.getCollectionsByCreator(userAddress)
      const collections: Collection[] = []
  ...
  onERC721CollectionCreated(
    callback: (creator: string, collection: string, name: string, symbol: string) => void
  ): void {
